...
const UI = {
  ku: { addNote: "تێبینی زیاد بکە...", confirmTitle: "داواکارییەکەت نێردرا! ✓", confirmMsg: "خزمەتگوزار دێتەوە بۆ میزەکەت.", popular: "بەناوبانگ", total: "کۆی گشتی", pieces: "دانە", yourOrder: "داواکاریەکەت", tableNo: "ژمارەی مێز", sendOrder: "داواکاری بنێرە", loading: "بارکردن...", sending: "ناردن...", notFound: "چێشتخانە نەدۆزرایەوە" },
  ar: { addNote: "أضف ملاحظة...", confirmTitle: "تم إرسال طلبك! ✓", confirmMsg: "سيأتي النادل إلى طاولتك.", popular: "الأكثر طلباً", total: "المجموع", pieces: "قطعة", yourOrder: "طلبك", tableNo: "رقم الطاولة", sendOrder: "إرسال الطلب", loading: "جار التحميل...", sending: "جار الإرسال...", notFound: "المطعم غير موجود" },
  en: { addNote: "Add a note...", confirmTitle: "Order Sent! ✓", confirmMsg: "Your waiter is on the way.", popular: "Popular", total: "Total", pieces: "pcs", yourOrder: "Your Order", tableNo: "Table", sendOrder: "Place Order", loading: "Loading...", sending: "Sending...", notFound: "Restaurant not found" },
};
...
export default function TenantMenu() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      setRestaurant(rest);
      setLang(rest.locale || "ku");

      // 2. Load categories, items, table — all scoped to this restaurant
      ...
      const [catRes, itemRes, tableRes] = await Promise.all([
        ...
        tableToken
          ? supabase.from("tables").select("*").eq("restaurant_id", rest.id).eq("qr_token", tableToken).single()
          : Promise.resolve({ data: null }),
      ]);

      setCategories(catRes.data || []);
      setItems(itemRes.data || []);
      setTableInfo(tableRes.data || { label: "—", id: null });
  ...
  const placeOrder = async () => {
    if (!tableInfo?.id || cartEntries.length === 0 || !restaurant) return;
    setSending(true);
  ...
  const fmt = (p) => {
    ...
    const cur = restaurant?.currency === "IQD" ? (lang === "en" ? "IQD" : "دینار") : restaurant?.currency || "";
    return `${num} ${cur}`;
  };

  if (notFound) {
    return (
      <div style={{ background: "#0f0f0f", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", color: "#666", fontFamily: "sans-serif", textAlign: "center", padding: 20 }}>
        <div>
          <div style={{ fontSize: 48, marginBottom: 16 }}>🔍</div>
          <div style={{ fontSize: 18, fontWeight: 700, color: "#f5f0e8" }}>Restaurant not found</div>
          <div style={{ fontSize: 14, marginTop: 8, color: "#555" }}>Check the URL and try again</div>
        </div>
      </div>
    );
  }
    ...
    <div dir={dir} style={{ fontFamily: isRTL ? "'Noto Sans Arabic',sans-serif" : "'Inter',sans-serif", background: "#0f0f0f", minHeight: "100vh", color: "#f5f0e8", maxWidth: 480, margin: "0 auto", position: "relative", overflowX: "hidden" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .add-btn{background:${accent};border:none;color:#0f0f0f;border-radius:50px;width:36px;height:36px;font-size:22px;cursor:pointer;display:flex;align-items:center;justify-content:center;font-weight:700;flex-shrink:0;transition:transform 0.15s}
        ...
        .lang-btn.active{background:${accent};color:#0f0f0f;border-color:${accent};font-weight:600}
        ...
        .cat-btn.active{background:${accent};color:#0f0f0f;border-color:${accent};font-weight:600}
        .popular-badge{background:${accent}22;color:${accent};font-size:11px;padding:2px 8px;border-radius:20px;border:1px solid ${accent}44;font-weight:500}
        .cart-fab{position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:${accent};color:#0f0f0f;border:none;border-radius:50px;padding:14px 28px;font-size:16px;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:10px;box-shadow:0 4px 24px ${accent}66;z-index:50;white-space:nowrap;width:calc(100% - 48px);max-width:400px;justify-content:space-between}
        ...
        .place-btn{background:${accent};color:#0f0f0f;border:none;border-radius:14px;padding:16px;font-size:17px;font-weight:700;cursor:pointer;width:100%;font-family:inherit}
      ...
      <div style={{ background: `linear-gradient(180deg, ${accent}22 0%, #0f0f0f 100%)`, padding: "24px 16px 0", borderBottom: "1px solid #222" }}>
        ...
        <div style={{ textAlign: "center", paddingBottom: 20 }}>
          <div style={{ fontSize: 38, marginBottom: 6 }}>{restaurant?.logo_emoji || "🍽️"}</div>
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#f5f0e8", marginBottom: 4 }}>{restaurant?.name}</h1>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: `${accent}18`, border: `1px solid ${accent}44`, borderRadius: 20, padding: "5px 14px", fontSize: 13, color: accent }}>
        ...
        {filteredItems.map((item) => {
          ...
          return (
            <div key={item.id} className="item-card">
              <div className="emoji-plate" style={item.image_url ? {padding:0,overflow:"hidden"} : {}}>
                  {item.image_url ? <img src={item.image_url} alt="" style={{width:"100%",height:"100%",objectFit:"cover",borderRadius:14}}/> : item.image_emoji}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <span style={{ fontSize: 17, fontWeight: 700, color: accent }}>{fmt(item.price)}</span>
                  {qty === 0 ? (
        ...
        <>
          ...
          <div className="cart-sheet">
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 20px 16px" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, background: `${accent}22`, border: `1px solid ${accent}44`, borderRadius: 20, padding: "4px 12px", color: accent, fontSize: 13 }}>
                🪑 {tableInfo?.label}
              </div>
            </div>
                  ...
                  <div key={id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 0", borderBottom: "1px solid #222" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 13, color: accent }}>{fmt(item.price)}</div>
                      </div>
                    </div>
                    <div className="qty-ctrl">
            ...
            <div style={{ display: "flex", justifyContent: "space-between", padding: "16px 20px", fontSize: 18, fontWeight: 700 }}>
              <span>{t.total}</span>
              <span style={{ color: accent }}>{fmt(cartTotal)}</span>
            </div>
